...
# Script to setup cronjob for producer PubSub mode

# Configuration
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
RUN_SCRIPT="$SCRIPT_DIR/run-producer-pubsub.sh"
CRON_COMMENT="CKG Producer PubSub Mode"
CRON_JOB="0 */12 * * * $RUN_SCRIPT  # Run every 12 hours"
...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  add     - Add the producer PubSub cronjob (runs every 12 hours)"
        echo "  remove  - Remove the producer PubSub cronjob"
        echo "  status  - Check if the cronjob is active"
        echo "  list    - List all current crontab entries"
        echo ""
        echo "Default schedule: Every 12 hours (0 */12 * * *)"
        echo "Log file: $PROJECT_ROOT/logs/producer-pubsub-cron.log"
        exit 1
        ;;
esac