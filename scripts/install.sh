...
0 2 * * * root $TARGET_DIR/scripts/producer/ckg-producer 2>&1

# Update Service
# Check for git updates every 15 minutes and pull if there are changes
0 0 * * * root $TARGET_DIR/scripts/install.sh update >> $LOG_DIR/ckg-update.log 2>&1
EOF
...
print_instructions() {
    ...
    echo "    View log (API):     sudo tail -f $LOG_DIR/producer-api.log"
    echo "    View log (PubSub):  sudo tail -f $LOG_DIR/producer-pubsub.log"
